...
async function retry(fn, tries = 3, delays = [500, 1000, 2000]) {
    ...
    for (let i = 0; i < tries; i++) {
        try {
            ...
            lastErr = e;
            if (i < tries - 1) await sleep(delays[i] || 1000);
        }
    }
    throw lastErr;
}
...
async function loadCookies(page) {
    try {
        ...
        logError(`threads cookies load failed: ${e?.message}`);
    }
}
async function saveCookies(page) {
    try {
        const all = await getAllCookies(page);
        const forThreads = all.filter((c) =>
            /threads\.(net|com)$/i.test((c.domain || "").replace(/^\./, ""))
        );
        fs.writeFileSync(COOKIES_THREADS_PATH, JSON.stringify(forThreads, null, 2));
        logStep("Cookies saved (threads)");
    } catch (e) {
        logError(`cookies save failed: ${e?.message}`);
    }
}
...
async function isThreadsAuthorized(page) {
    return await page
        .evaluate(
            (PROFILE, SEL_COMPOSER) => {
                const hasComposer = Array.from(
                    document.querySelectorAll(SEL_COMPOSER)
                ).some((el) =>
                    /Що нового\?|What’s new\?|What's new\?/i.test(
                        el.textContent || ""
                    )
                );
...
async function fillThreadsLoginForm(page, user, pass) {
    ...
    const timeout = 10000;
    const u = await page
        .waitForSelector(uSel, { timeout })
        .catch((e) => {
            logError(
                `[fillThreadsLoginForm] wait for user input failed: ${e?.message}`
            );
            return null;
        });
    const p = await page
        .waitForSelector(pSel, { timeout })
        .catch((e) => {
            logError(
                `[fillThreadsLoginForm] wait for pass input failed: ${e?.message}`
            );
            return null;
        });
    ...
    let handle = await page
        ...
        .catch(() => null);
    let loginBtn = handle?.asElement?.();

    if (!loginBtn) {
        const cssBtn = "input[type='submit'], div[role='button']";
        const xpathBtn = "//*[contains(text(),'Увійти')]";
        console.log("[fillThreadsLoginForm] fallback button search", {
            cssBtn,
            xpathBtn,
        });
        loginBtn = await findWithFallback(page, cssBtn, xpathBtn);
    }
    ...
    const btnHtml = await page
        .evaluate((el) => el.outerHTML, loginBtn)
        .catch(() => null);
...
export async function ensureThreadsReady(page, opts = {}) {
    const { user: threadsUser, pass: threadsPass } = {
        ...getThreadsCreds(),
        ...opts,
    };
    ...
    await tryStep(
        ...
        async () => {
            ...
            await retry(async () => {
                let resp = await page.goto(loginUrl, {
                    waitUntil: "domcontentloaded",
                    timeout: 30000,
                });
    ...
    const already = await tryStep(
        "Check threads auth",
        () => isThreadsAuthorized(page),
        { page }
    );
    if (already) {
    ...
    await tryStep(
        "threads login",
        () => fillThreadsLoginForm(page, threadsUser, threadsPass),
        { page }
    );
    ...
    if ((page.url() || "").includes("/login")) {
        ...
        const apiRes = await tryStep(
            "threads api login",
            () => loginViaApi(page, threadsUser, threadsPass),
            { page }
        );
        if (apiRes?.authenticated) {
            const redirect =
                apiRes.redirect_uri || "https://www.threads.net/";
            await tryStep(
                "api login redirect",
                () =>
                    retry(() =>
    ...
    await tryStep(
        "Очікую завантаження фіду Threads…",
        () => waitUrlNot(page, "/login", 45000),
        { page }
    );
    ...
    if ((page.url() || "").includes("/login")) {
        ...
        const e = new Error(
            "Threads: після логіну залишилися на сторінці логіну."
        );
        e.keepOpen = true;
        throw e;
    }
    ...
    while (Date.now() < until) {
        if (await isThreadsAuthorized(page)) {
            ...
            logStep(
                "Threads готовий (авторизовано, є «Що нового?»)"
            );
            break;
        }
        await sleep(500);
    ...
    if (!(await isThreadsAuthorized(page))) {
        ...
        fs.writeFileSync(
            path.resolve("коди сторінок", "threads_not_authorized.html"),
            dom
        );
        await takeShot(page, "threads_not_authorized");
        const e = new Error(
            "Threads: не бачу авторизованої головної (композер/Опублікувати/аватар)."
        );
        e.keepOpen = true;
        throw e;
    }
